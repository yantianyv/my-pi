...
- **模型可选（/btw-config）**：默认 `auto` = 当前已认证可用模型中最便宜的（input+output 单价合计，同价按 id 序），并按价格从低到高**故障转移**——最便宜模型调用失败（认证/网络/API 错误）自动换下一个更贵的模型重试，全部失败才报错；`/btw-config` 不带参数弹出选择列表（auto + 最便宜的 10 个模型），或直接 `/btw-config auto` / `/btw-config provider/modelId` 指定；开面板时提示当前生效模型；
