...
src/                # 全部源码 / 原始素材 + npm 生态 + 构建脚本（build.js 的唯一输入）
  ...
  extensions/       #   扩展源码（产物 dist/extensions/ 由 build.js 生成）
    ...
    web-tool.ts       #   联网工具：web_search 多源搜索（bing.cn 主 + 360 备 + npm 垂类，零 key 零费用，无 AI 总结）+ web_fetch 抓网页转 markdown（正文提取 + 截断；turndown/domino/gfm 由 build.js 内联）+ /web-tool-config 代理设置面板；降级链：直连 → 代理 → 系统 curl（绕过对 Node TLS 指纹的 301 挑战）
