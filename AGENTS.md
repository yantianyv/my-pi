...
pi（@earendil-works/pi-coding-agent）的个人定制配置仓库：主题、扩展、提示音集中管理，`install.js` 一键安装到全局配置目录 `~/.pi/agent/`。**伪编译架构**：源码层高复用（`extensions/shared/` 共享模块），`build.js` 用 esbuild 把每个扩展入口打包成 `dist/extensions/` 下的**零耦合单文件**（运行时由 pi 经 jiti 直接加载，不经过 tsc/构建产物转换）。

## 常用命令

| 命令 | 作用 | 出处 |
|---|---|---|
| `node install.js` | 安装 themes/extensions/sounds 到 `~/.pi/agent/`，并把 settings.json 的 theme 设为 matrix（install.js:129） | install.js |
| `node install.js --dry-run`（或 `-n`） | 试运行，只打印不修改 | install.js:33 |
| `node build.js` | 伪编译：esbuild 把 extensions/ 源码（含 shared/、hud/ 子模块）内联打包成 dist/extensions/ 下的零耦合单文件（hud/ → hud.ts）；改了源码后重跑 | build.js |
| `npm install` | 首次拉取构建依赖（esbuild，devDependency）；无 package.json 时需要 | package.json |
...
```
install.js          # 安装脚本：优先从 dist/extensions/（伪编译产物）复制 .ts/.json/.wav 到 ~/.pi/agent/ 对应子目录，未构建时回退 extensions/ 源码；并生成 tsconfig.json（探测 pi 全局目录）
build.js            # 伪编译脚本：esbuild 把每个扩展入口 + 共享模块内联打包成 dist/extensions/ 单文件（零耦合）
package.json        # 构建工具声明（esbuild devDependency；npm install 拉取）；非运行时依赖
package-lock.json   # npm 锁定文件（入库）
tsconfig.build.json # build.js 专用：无 paths 的构建 tsconfig（主 tsconfig.json 的 paths 会破坏 packages: external）
.gitignore          # 忽略生成物 tsconfig.json / node_modules
README.md           # 项目说明（含 HUD 图例、各扩展用法、卸载方法）
dist/extensions/    # 伪编译产物（入库）：每扩展一个零耦合单文件 .ts（hud.ts 由 hud/ 合并而来）；改源码后 build 再提交
  btw.ts            #   内含 shared/model-select.ts 内联（btw 与 explore 共用模型选择器）
  hud.ts            #   hud/ 五个子模块合并为单文件（解决 hud 拆分问题）
  ...               #   其余扩展与源码同名
  shared/           #   （不入产物，见下）
extensions/         # pi 扩展源码（安装目标 ~/.pi/agent/extensions/，产物由 build.js 生成）
...
- **伪编译架构**：源码（extensions/，含 shared/ 共享模块与 hud/ 子目录）→ `build.js`（esbuild bundle，`packages: external` 保持 pi 官方包外部引用、相对 import 全部内联）→ `dist/extensions/` 单文件产物（零耦合、自包含）。`install.js` **优先安装产物**（dist 存在时），未构建回退源码。产物入库（克隆即用，`node install.js` 直接可用）；改了源码后需 `node build.js` 重建并同步提交 dist，否则 install.js 装的是旧产物。构建用 `tsconfig.build.json`（无 paths）——主 tsconfig 的 paths 会把包名解析成 pi 全局绝对路径，导致 `packages: external` 包名匹配失效、意外内联 typebox。
- **安装模型**：仓库即源码 + 产物，`install.js` 按扩展名（`.ts`/`.json`/`.wav`）复制到 `~/.pi/agent/` 下的 `themes/`、`extensions/`、`sounds/`；改扩展后必须重跑 build.js（如改源码）+ install.js + pi 内 `/reload`。
...
- **改完扩展不重装不生效**：源码在仓库，运行时是 `~/.pi/agent/extensions/` 的副本（伪编译后为 dist 产物副本），两处易不同步。改动流程：改 `extensions/` 源码 → `node build.js` → `node install.js` → pi 内 `/reload`。
...
- `docs/deepseek/` 是本地参考资料（不入库，版权归 DeepSeek），不要当作可执行配置；`sounds/` 只放提示音
