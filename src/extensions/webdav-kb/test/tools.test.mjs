...
try {
	...
	r = await tool("kb_write").execute("9", { path: "/notes/webdav分类/webdav-坑.md", content: newNote }, undefined, undefined, ctx);
	check("kb_write 新建成功", r.content[0].text.includes("已写入"), r.content[0].text.slice(0, 60));
	check("kb_write 同步上传远端", dav.store.has(dav.prefix + "/notes/webdav分类/webdav-坑.md"));

	// ---- kb_write 查重守卫 ----
	r = await tool("kb_write").execute("10", { path: "/notes/webdav分类/webdav-坑.md", content: newNote }, undefined, undefined, ctx);
	check("kb_write 已存在需 overwrite", r.content[0].text.includes("overwrite"), r.content[0].text.slice(0, 80));
	r = await tool("kb_write").execute("11", { path: "/notes/webdav分类/webdav-坑.md", content: newNote.replace("123 云盘", "坚果云"), overwrite: true }, undefined, undefined, ctx);
	check("kb_write overwrite:true 覆盖", r.content[0].text.includes("已写入"), r.content[0].text.slice(0, 60));

	// ---- kb_append ----
	r = await tool("kb_append").execute("12", { path: "/notes/webdav分类/webdav-坑.md", content: "补充：不要用 PUT 传大文件。" }, undefined, undefined, ctx);
	check("kb_append 成功", r.content[0].text.includes("已追加"), r.content[0].text.slice(0, 60));
	r = await tool("kb_read").execute("13", { path: "/notes/webdav分类/webdav-坑.md" }, undefined, undefined, ctx);
	...
	check("kb_list 列出目录", r.content[0].text.includes("📁 /notes") && r.content[0].text.includes("📄 /notes/webdav分类/webdav-坑.md"), r.content[0].text.slice(0, 80));
	...
	// 未解锁时写 vault → 应报错
	r = await tool("kb_write").execute("15", { path: "/vault/密分类/密.md", content: "---\ntitle: 密\ntags: []\n---\n机密内容\n" }, undefined, undefined, ctx);
	check("未解锁写 vault 报错", r.content[0].text.includes("失败"), r.content[0].text.slice(0, 80));
	// 解锁后写读
	cryptoMod.unlockVault("口令x", setup);
	r = await tool("kb_write").execute("16", { path: "/vault/密分类/密.md", content: "---\ntitle: 密\ntags: []\n---\n机密内容：勿外传。\n" }, undefined, undefined, ctx);
	check("解锁后写 vault 成功", r.content[0].text.includes("已写入"), r.content[0].text.slice(0, 60));
	check("vault 明文未落盘", !existsSync(join(mirrorDir, "vault", "密分类", "密.md")));
	check("vault .enc 已落盘", existsSync(join(mirrorDir, "vault", "密分类", "密.md.enc")));
	r = await tool("kb_read").execute("17", { path: "/vault/密分类/密.md" }, undefined, undefined, ctx);
	check("解锁后读 vault 解密", r.content[0].text.includes("勿外传"), r.content[0].text.slice(0, 60));

	// ---- vault 锁定后读报错 ----
	cryptoMod.lockVault();
	r = await tool("kb_read").execute("18", { path: "/vault/密分类/密.md" }, undefined, undefined, ctx);
	...
	// ---- kb_read 分页（offset） ----
	const longNote = "---\ntitle: 长文\ntags: []\n---\n" + "字".repeat(100);
	await tool("kb_write").execute("20", { path: "/notes/长文分类/长文.md", content: longNote }, undefined, undefined, ctx);
	r = await tool("kb_read").execute("21", { path: "/notes/长文分类/长文.md", offset: 20 }, undefined, undefined, ctx);
	...
	r = await tool("kb_read").execute("22", { path: "/notes/长文分类/长文.md", offset: 99999 }, undefined, undefined, ctx);
	...
	// ---- kb_delete ----
	// 未确认拒绝
	r = await tool("kb_delete").execute("26", { path: "/notes/长文分类/长文.md" }, undefined, undefined, ctx);
	check("kb_delete 需 confirm", r.content[0].text.includes("confirm:true"), r.content[0].text.slice(0, 60));
	// 确认删除
	r = await tool("kb_delete").execute("27", { path: "/notes/长文分类/长文.md", confirm: true }, undefined, undefined, ctx);
	check("kb_delete 删除成功", r.content[0].text.includes("已删除"), r.content[0].text.slice(0, 60));
	check("kb_delete 本地镜像删", !existsSync(join(mirrorDir, "notes", "长文分类", "长文.md")));
	check("kb_delete 远端删", !dav.store.has(dav.prefix + "/notes/长文分类/长文.md"));
	check("kb_delete 审计日志", existsSync(join(mirrorDir, ".kb-delete-log")));
	check("kb_delete 账本清条目", !JSON.parse(readFileSync(join(mirrorDir, ".kb-sync.json"), "utf8")).files["/notes/长文分类/长文.md"]);
	// lfs 删除引导
