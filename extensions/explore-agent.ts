...
function buildSystemPrompt(cwd: string): string {
	return [
		...
		"3. 完成后输出精炼的中文报告：直接回答任务问题，给出关键文件路径（必要时带行号）与重要结论；没找到或不确定的地方要明确说明",
		"4. 只输出报告本身，不要客套话，不要粘贴大段代码",
	].join("\n");
}
...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		promptGuidelines: [
			...
			"explore 的任务描述要具体（如『搞清 auth 模块的登录流程，给出关键文件与函数』）；多个相互独立的任务放在一次调用里并行执行。",
		],
